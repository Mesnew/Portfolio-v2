...
export function SolarSystem3D() {
  ...
  const [hoverTransition, setHoverTransition] = useState<{ [key: string]: number }>({})
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 3)) // Increased max pixel ratio for better quality
    ...
    const bloomPass = new UnrealBloomPass(
      ...
      0.3, // Reduced bloom strength to maintain clarity during zoom
      0.6, // Adjusted radius for better quality at all zoom levels
      0.7, // Higher threshold to preserve detail
    )
    ...
    controls.dampingFactor = 0.03 // Smoother damping for better zoom experience
    controls.minDistance = 5 // Allow closer zoom
    controls.maxDistance = 300 // Extended max distance
    controls.maxPolarAngle = Math.PI
    controls.minPolarAngle = 0
    controls.autoRotate = false
    controls.autoRotateSpeed = 0.5
    controls.zoomSpeed = 0.6 // Optimized zoom speed
    ...
    const createStarfield = () => {
      ...
      const starMaterial = new THREE.ShaderMaterial({
        uniforms: {
          uPixelRatio: { value: Math.min(window.devicePixelRatio, 2) }, // Higher pixel ratio for stars
          uTime: { value: 0 },
        },
        vertexShader: `
    ...
    void main() {
      ...
      gl_PointSize = size * uPixelRatio * (400.0 / distance) * twinkle * clamp(distance / 50.0, 0.5, 2.0);
    }
  `,
        fragmentShader: `
    varying vec3 vColor;
  ...
  `,
    ...
    const createInteractiveElements = () => {
      ...
      // Satellites artificiels en orbite autour de la Terre
      ...
      for (let i = 0; i < 5; i++) {
        ...
        satellite.position.set(Math.cos(angle) * radius, (Math.random() - 0.5) * 2, Math.sin(angle) * radius)
        satellite.userData = { type: "satellite", orbitRadius: radius, orbitSpeed: 0.02 + Math.random() * 0.01 }
        elements.push(satellite)
      }

      // Comètes avec queues lumineuses
      for (let i = 0; i < 3; i++) {
        const cometGroup = new THREE.Group()

        // Corps de la comète
        const cometGeometry = new THREE.SphereGeometry(0.5, 8, 8)
        const cometMaterial = new THREE.MeshBasicMaterial({ color: 0x888888 })
        const cometCore = new THREE.Mesh(cometGeometry, cometMaterial)
        cometGroup.add(cometCore)

        // Queue de la comète
        const tailGeometry = new THREE.ConeGeometry(0.2, 8, 8)
        ...
        // Position aléatoire lointaine
        const distance = 100 + Math.random() * 200
        const angle = Math.random() * Math.PI * 2
        cometGroup.position.set(Math.cos(angle) * distance, (Math.random() - 0.5) * 50, Math.sin(angle) * distance)
        ...
        elements.push(cometGroup)
      }

      // Stations spatiales lointaines
      for (let i = 0; i < 2; i++) {
        const stationGroup = new THREE.Group()

        // Structure principale
        const mainGeometry = new THREE.CylinderGeometry(1, 1, 4, 8)
        const mainMaterial = new THREE.MeshBasicMaterial({ color: 0x666666 })
        ...
        const panelMaterial = new THREE.MeshBasicMaterial({ color: 0x001144 })
        ...
        const angle = Math.random() * Math.PI * 2
        stationGroup.position.set(Math.cos(angle) * distance, (Math.random() - 0.5) * 30, Math.sin(angle) * distance)
        ...
        elements.push(stationGroup)
      }

      // Sondes spatiales en mouvement
      for (let i = 0; i < 4; i++) {
        const probeGroup = new THREE.Group()

        // Corps de la sonde
        const probeGeometry = new THREE.BoxGeometry(0.8, 0.4, 1.2)
        const probeMaterial = new THREE.MeshBasicMaterial({ color: 0xcccccc })
        const probeBody = new THREE.Mesh(probeGeometry, probeMaterial)
        probeGroup.add(probeBody)

        // Antenne
        const antennaGeometry = new THREE.CylinderGeometry(0.02, 0.02, 2, 4)
        ...
        const angle = Math.random() * Math.PI * 2
        probeGroup.position.set(Math.cos(angle) * distance, (Math.random() - 0.5) * 40, Math.sin(angle) * distance)

        probeGroup.userData = {
          type: "probe",
          trajectory: {
            center: probeGroup.position.clone(),
            radius: 20 + Math.random() * 30,
            speed: 0.003 + Math.random() * 0.005,
            phase: Math.random() * Math.PI * 2,
          },
        }
        elements.push(probeGroup)
      }
    ...
    interactiveElements.forEach((element) => scene.add(element))
    ...
    const handleResize = () => {
      ...
      rendererRef.current.setPixelRatio(Math.min(window.devicePixelRatio, 3)) // Consistent high pixel ratio
      ...
      if (starMaterial.uniforms) {
        starMaterial.uniforms.uPixelRatio.value = Math.min(window.devicePixelRatio, 2) // Higher pixel ratio for stars
      }
      if (distantStarMaterial && distantStarMaterial.uniforms) {
        distantStarMaterial.uniforms.uPixelRatio.value = Math.min(window.devicePixelRatio, 2)
      }
    ...
    const animate = () => {
      ...
      interactiveElements.forEach((element) => {
        ...
        if (userData.type === "satellite") {
          // Orbit around Earth (assuming Earth is at origin for simplicity)
          userData.phase = (userData.phase || 0) + userData.orbitSpeed
          element.position.x = Math.cos(userData.phase) * userData.orbitRadius
          element.position.z = Math.sin(userData.phase) * userData.orbitRadius
          element.rotation.y += 0.05
        }

        if (userData.type === "comet") {
          // Move comet along its trajectory
          element.position.add(userData.direction)
          element.lookAt(new THREE.Vector3(0, 0, 0))

          // Reset position if too far
          if (element.position.length() > 500) {
            const distance = 100 + Math.random() * 200
            const angle = Math.random() * Math.PI * 2
            element.position.set(Math.cos(angle) * distance, (Math.random() - 0.5) * 50, Math.sin(angle) * distance)
          }
        }
        ...
        if (userData.type === "station") {
          ...
          element.rotation.z += userData.rotationSpeed * 0.5

          // Blinking lights
          userData.blinkTimer += 1
          const light = element.children.find((child) => child.material?.color?.r === 1)
          if (light && userData.blinkTimer % 60 < 30) {
            light.material.color.setHex(0xff0000)
          } else if (light) {
            light.material.color.setHex(0x440000)
          }
        }

        if (userData.type === "probe") {
          // Move probe in orbital pattern
          const trajectory = userData.trajectory
          trajectory.phase += trajectory.speed

          element.position.x = trajectory.center.x + Math.cos(trajectory.phase) * trajectory.radius
          element.position.y = trajectory.center.y + Math.sin(trajectory.phase * 0.7) * (trajectory.radius * 0.3)
          element.position.z = trajectory.center.z + Math.sin(trajectory.phase) * trajectory.radius
      ...
      if (cameraRef.current && sceneRef.current) {
        ...
        if (intersects.length > 0) {
          ...
          // Enhanced glow effect on hover with smooth transitions
          if (planet.children.length > 0) {
            planet.children.forEach((child) => {
              if (child.material && child.material.transparent && !(child instanceof THREE.Sprite)) {
                const targetOpacity = 0.8
                const targetGlowScale = 1.5
                child.material.opacity = THREE.MathUtils.lerp(child.material.opacity, targetOpacity, 0.15)
                const currentGlowScale = child.scale.x
                const newGlowScale = THREE.MathUtils.lerp(currentGlowScale, targetGlowScale, 0.12)
                child.scale.setScalar(newGlowScale)
              }
            })
          }
        } else {
          planetsRef.current.forEach((planet) => {
            const currentScale = planet.scale.x
            const newScale = THREE.MathUtils.lerp(currentScale, 1.0, 0.08)
            planet.scale.setScalar(newScale)
            ...
            if (planet.children.length > 0) {
              planet.children.forEach((child) => {
                if (child.material && child.material.transparent && !(child instanceof THREE.Sprite)) {
                  ...
                  const currentGlowScale = child.scale.x
                  const newGlowScale = THREE.MathUtils.lerp(currentGlowScale, 1.0, 0.08)
                  child.scale.setScalar(newGlowScale)
                }
              })
            }
  ...
  return (
    <>
      ...
      {hoveredPlanet && (
        <div
          className="fixed pointer-events-none z-50 bg-black/80 backdrop-blur-sm text-white px-4 py-2 rounded-lg text-sm border border-primary/30 shadow-lg transition-all duration-300 ease-out"
          style={{
      ...
      <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-10">
        {hoveredPlanet && (
          <div className="bg-black/60 backdrop-blur-md text-white px-6 py-3 rounded-full text-sm flex items-center border border-primary/20 shadow-xl animate-in slide-in-from-top-2 duration-300">
            <div className="w-3 h-3 bg-primary rounded-full mr-3 animate-pulse shadow-lg shadow-primary/50"></div>
            <span className="font-medium">Planète: {hoveredPlanet}</span>
            <div className="ml-3 text-xs text-primary/80">Double-clic pour explorer</div>
          </div>
        )}
      </div>
    </>
